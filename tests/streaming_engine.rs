...
//!
//! These tests verify that the streaming engine (via
//! `EngineBuilder::with_engine(EngineKind::Streaming)` and
//! `EngineKind::Auto` over a strip source) produces **pixel-exact** output
//! matching the monolithic engine (`EngineKind::Monolithic`) across all
//! supported layouts, tile sizes, and image dimensions.
//!
...
//!
//! - **Auto-selection** — verifies that `EngineKind::Auto` picks the
//!   monolithic path for an in-memory raster and the streaming path when
//!   given a strip source.
//!
...
use libviprs::{
    BlankTileStrategy, CollectingObserver, EngineBuilder, EngineConfig, EngineEvent, EngineKind,
...
/// Run auto-selecting engine (over a strip source) and return sorted tiles.
fn auto_tiles(
    src: &Raster,
    plan: &libviprs::PyramidPlan,
    memory_budget_bytes: u64,
    ...
    let strip_src = RasterStripSource::new(src);
    EngineBuilder::new(strip_src, plan.clone(), &sink)
        // Auto over a strip source resolves to Streaming.
        .with_memory_budget(memory_budget_bytes)
        .with_budget_policy(BudgetPolicy::Error)
        .run()
        .unwrap();
...
fn streaming_parity_deepzoom_512x384() {
    ...
    let test_tiles = streaming_tiles(&src, &plan, 500_000, EngineConfig::default());
...
fn streaming_parity_deepzoom_300x200() {
    ...
    let test_tiles = streaming_tiles(&src, &plan, 400_000, EngineConfig::default());
...
fn streaming_parity_deepzoom_odd_dimensions() {
    ...
    for (w, h) in [(500, 300), (1023, 769), (257, 129)] {
        ...
        let test_tiles = streaming_tiles(&src, &plan, 2_500_000, EngineConfig::default());
        assert_tiles_match(
            &ref_tiles,
            &test_tiles,
...
fn auto_selects_monolithic_for_large_budget() {
    ...
    let ref_tiles = monolithic_tiles(&src, &plan, &EngineConfig::default());
    // Auto over a Raster resolves to Monolithic regardless of budget.
    let sink = MemorySink::new();
    EngineBuilder::new(&src, plan.clone(), &sink)
        // Auto (default)
        ...
        .run()
        .unwrap();
    let mut test_tiles = sink.tiles();
    test_tiles.sort_by_key(|t| (t.coord.level, t.coord.row, t.coord.col));
...
fn auto_selects_streaming_for_tiny_budget() {
    ...
    // Feed a strip source so Auto resolves to Streaming.
    let test_tiles = auto_tiles(&src, &plan, 1_200_000);
    assert_eq!(test_tiles.len() as u64, plan.total_tile_count());
}
...
fn streaming_deterministic_across_tile_sizes() {
    ...
    for tile_size in [64, 128, 256] {
        ...
        let test_tiles = streaming_tiles(&src, &plan, 700_000, EngineConfig::default());
...
fn streaming_parity_google_centre_small() {
    ...
    let test_tiles = streaming_tiles(&src, &plan, 1_000_000, EngineConfig::default());
...
fn streaming_blank_tile_placeholder_solid_white() {
    ...
    let ref_result = EngineBuilder::new(&src, plan.clone(), &ref_sink)
        .with_engine(EngineKind::Monolithic)
        .with_config(ref_config)
        .run()
        .unwrap();
...
fn streaming_peak_memory_lower_than_monolithic() {
    ...
    let mono_result = EngineBuilder::new(&src, plan.clone(), &mono_sink)
        .with_engine(EngineKind::Monolithic)
        .with_config(EngineConfig::default())
        .run()
        .unwrap();
    ...
    let strip_src = RasterStripSource::new(&src);
    let stream_result = EngineBuilder::new(strip_src, plan.clone(), &streaming_sink)
        .with_engine(EngineKind::Streaming)
        .with_config(EngineConfig::default())
        .with_memory_budget(5_000_000)
        .with_budget_policy(BudgetPolicy::Error)
        .run()
        .unwrap();
...
fn streaming_memory_savings_scale_with_image_size() {
    ...
    for &(w, h) in cases {
        ...
        let strip_src = RasterStripSource::new(&src);
        let stream_result = EngineBuilder::new(strip_src, plan.clone(), &stream_sink)
            .with_engine(EngineKind::Streaming)
            .with_config(EngineConfig::default())
            .with_memory_budget(min_budget * 5 / 4)
            .with_budget_policy(BudgetPolicy::Error)
            .run()
            .unwrap();
