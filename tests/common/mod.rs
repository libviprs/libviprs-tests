//! Shared test helpers for the PDF / pdfium suite.
//!
//! Per Rust convention, this is a non-test module that integration tests
//! pull in via `mod common;` (the directory layout `tests/common/mod.rs`
//! prevents Cargo from compiling it as its own integration target). Used
//! by:
//!
//! - `pdfium_streaming_render.rs`
//! - `pdfium_streaming_rotation_matrix.rs`
//! - `pdfium_streaming_memory.rs`
//! - `pdfium_streaming_synthetic_rotations.rs`
//! - `pdfium_streaming_perf_smoke.rs`
//! - `pdfium_streaming_concurrent.rs`
//! - `pdfium_streaming_regression_rotate.rs`
//! - `pdfium_streaming_determinism.rs`
//!
//! Centralising these helpers eliminates the byte-for-byte duplication
//! that previously lived in each test file.

#![allow(dead_code)] // Each test file uses a subset; allow per-target unused.
#![cfg(feature = "pdfium")]
...
/// Per-channel mean tolerance for region-correctness comparisons within a
/// single pdfium render path (matrix vs matrix, or form-data vs form-data).
///
/// Origin: empirically measured in `streaming_pdfium_matrix.rs` (see its
/// rationale comment lines 13-22). pdfium's matrix path is bitmap-size-
/// dependent in its anti-aliasing — strip(K, h) and full[K..K+h] can have
/// up to 15 % of pixels differing with channel deltas up to 172/255 — but
/// the **mean** over a strip stays under 1.0 unit on our blueprint
/// fixtures. ±2.0 leaves comfortable headroom while still catching
/// gross matrix errors (wrong rotation, off-by-page-size shift,
/// transposed scale produce mean drift in the tens).
pub const REGION_MEAN_TOLERANCE: f64 = 2.0;

/// Per-channel mean tolerance for **cross-path** comparisons (form-data
/// vs matrix). pdfium's two render paths use independent rasterisers; the
/// AA divergence between them is real. Empirically up to 5 mean units
/// drift on the blueprint fixtures (see the failed first-pass synthetic
/// rotations test, where /Rotate 270 full-page diverged by 5.59). ±10.0
/// absorbs that without losing the ability to catch a wrong-region bug,
/// which produces drift in the tens.
...
/// Asserts two RGBA8 byte buffers cover the same page region within
/// [`CROSS_PATH_MEAN_TOLERANCE`]. Use when comparing across pdfium's two
/// render paths (cached form-data vs streaming matrix).
...
/// over passing custom tolerances per call site — calibrate the constant
/// once, link the empirical justification, reuse.
...
/// rasters whose reported widths drift by a few pixels (DIM_DRIFT_TOLERANCE_PX
/// territory).
#[must_use]
